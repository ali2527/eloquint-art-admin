...
import { FaSearch,FaEdit, FaFilter, FaCaretDown, FaTrash } from "react-icons/fa";
...
function ArticleCategoryManagement() {
  ...
  const handleRedirect = (key,value) => {
    if(key == "edit"){
        navigate("/article-category-management/" + key + "/" + value);
    }else{
  ...
  const handleStatus = async () => {
    try {
      ...
      console.log(index)
      const response = await Post(ARTICLECATEGORIES.edit + selectedCategory._id ,{status:!selectedCategory.status}, token,{});
  ...
  return (
    <Layout className="configuration">
        ...
        <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <Button
              ...
              className="mainButton primaryButton"
              onClick={() => navigate("/article-category-management/add")}
            >
      ...
      >
        ...
        Do You Want To  {selectedCategory?.status ? "Deactivate" : "Activate"} This Article Category?
        </Typography.Text>
      </Modal>
    </Layout>
  );

...
function Gallery() {
    ...
    const getAllAdminGallery = async (pageNumber, keyword, max, min, sbj, days) => {
      setLoading(true);
      try {
        const response = await Get(GALLERY.getAllAdminGallery ,token);
        setLoading(false);
  ...
  return (
    <Layout className="configuration">
      <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <h1 className="pageTitle">Admin Gallery</h1>
          </Col>
          <Col
            xs={24}
            md={12}

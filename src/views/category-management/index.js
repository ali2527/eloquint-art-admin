...
import { FaSearch,FaEdit, FaFilter, FaCaretDown, FaEye } from "react-icons/fa";
...
function CategoryManagement() {
  ...
  const handleRedirect = (key,value) => {
    navigate("/category-management/" + key + "/" + value);
  };
  ...
  const getUsers = async (pageNumber, pageSize, search, reset = false) => {
    setLoading(true);
    try {
      const response = await Get(CATEGORIES.get, token, {
        page: pageNumber
          ? pageNumber.toString()
